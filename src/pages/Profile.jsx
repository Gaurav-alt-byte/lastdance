...
import { Loader2, Ban, UserMinus, ShieldAlert, ShieldCheck } from "lucide-react";
  ...
  useEffect(() => {
    const loadSubscribers = async () => {
      ...
      try {
        ...
        setSubscriberList(Array.isArray(response.data) ? response.data : []);
      } catch (error) {
            ...
            {subscriberLoading ? (
              <div className="space-y-3">
                {Array.from({ length: 3 }).map((_, index) => (
                  <div key={index} className="h-16 animate-pulse rounded-2xl bg-zinc-800" />
                ))}
              </div>
              ...
              <div className="space-y-3">
                {subscriberList.map((item) => {
                  ...
                  return (
                    <div key={userId} className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-black/20 p-4 md:flex-row md:items-center md:justify-between">
                      <Link to={`/channel/${subscriber.username}`} className="flex min-w-0 items-center gap-4">
                        <img
                        ...
                        <div className="min-w-0">
                          <p className="truncate font-semibold text-white">@{subscriber.username || "user"}</p>
                          <p className="truncate text-sm text-zinc-400">{subscriber.fullname || "Subscriber"}</p>
                        </div>
                      </Link>
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <span
                          className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold ${
                            isBlocked ? "bg-red-500/20 text-red-300" : "bg-green-500/20 text-green-300"
                          }`}
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => handleSubscriberAction(userId, isBlocked ? "unblock" : "block")}
                          disabled={busy}
